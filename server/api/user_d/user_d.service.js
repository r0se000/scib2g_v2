...
const queryList = require('./user_d.sql');
...
class userService {
    ...
    async createUser(signUpData) {
        ...
        accountArray = [signUpData.name, signUpData.id, signUpData.pwd, signUpData.usercode, signUpData.phone1, signUpData.phone2, signUpData.phone3, signUpData.provideYN];
     ...
     */
    async userLogin(accountName, accountBirth) {
        let cryptoKey = await mysqlDB('selectOne', queryList.select_key_string, []);
        cryptoKey = cryptoKey.row.key_string;
        let bYear = accountBirth.slice(0, 4),
            bMonth = accountBirth.slice(4, 6),
            bDate = accountBirth.slice(6, 8)

        accountName = cryptoUtil.encrypt_aes(cryptoKey, accountName);
        bYear = cryptoUtil.encrypt_aes(cryptoKey, bYear);
        bMonth = cryptoUtil.encrypt_aes(cryptoKey, bMonth);
        bDate = cryptoUtil.encrypt_aes(cryptoKey, bDate);

        let result = {},
            isUser = await mysqlDB('selectOne', queryList.select_user_exist, [accountName, bYear, bMonth, bDate]);
