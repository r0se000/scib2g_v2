...
exports.select_user_exist =
    `SELECT * from user_info WHERE name=? AND birth_year=? AND birth_month=? AND birth_date=? 
    AND user_status='Y';`;
...
exports.select_user_id =
    `SELECT p_account_id
       FROM user_protector
      WHERE p_user_name = ?
        AND p_user_phone_first = ?
        AND p_user_phone_middle = ? 
        AND p_user_phone_last = ?;`;
...
exports.select_user_code =
    `SELECT p_user_code
      ...
      WHERE p_account_id = ?
        AND a_user_name = ?
        AND a_user_phone_first = ?
        AND a_user_phone_middle = ? 
        AND a_user_phone_last = ?;`;
