...
// 호출할 service 정의
const service = require('./p_apphome.service');
const svInstance = new service();

class p_appHomeController {

    async p_apphomeload(req, res, next) {
        ...
        return res.render('app/p_apphome/p_apphome', {
            "userCode": userCode,
    ...
    async emergencyTimeInput(req, res, next) {
        let { emCode, userCode, a_user_code, emCheckTime } = req.body;
        let emTimeInput = await svInstance.emTimeInput(emCode, userCode, a_user_code, emCheckTime);
        return res.send(emTimeInput)
    }

}

module.exports = p_appHomeController;