...
// 호출할 service 정의
const service = require('./user_p.service');
...
class UserController {
    ...
    // 보호자, 독거노인 ID 매칭 21.11.10
    async userMatching(req, res, next) {
        let { userName, bYear, bMonth, bDay, p_user_code } = req.body;
        let result = await svInstance.userMatching(userName, bYear, bMonth, bDay, p_user_code);
        return res.json(result);
    }
}
