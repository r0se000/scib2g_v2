    ...
    if (emergencyList.length != 0) {
        emergencyList.forEach(function(item, index) {
            emListHtml = "<tr onclick='event.cancelBubble = true;' class='em-user-list' " + item.user_code + " id='emCode" + item.emergency_id + "' onclick='event.cancelBubble = true;'>" +
                ...
                "<td id='em-check-btn' em-code='" + item.emergency_id + "' onclick='td(this)'>" + "<button type='button' class='btn btn-xs btn-rounded btn-danger' id='clear'>응급확인</button>" +
                "</td></tr>";
    ...
    if (checkList.length != 0) {
        checkList.forEach(function(item, index) {
            checkListHtml = "<tr onclick='event.cancelBubble = true;' class='em-user-list' " + item.user_code + " id='emCode" + item.emergency_id + "' onclick='event.cancelBubble = true;'>" +
                ...
                "<td id='check-check-btn' user-code='" + item.user_code + "' em-code='" + item.emergency_id + "'onclick='check(this)'>" + "<button type='button' class='btn btn-xs btn-rounded btn-danger' id='clear2'>응급확인</button>" + "</td>" +
                "</td></tr>";
    ...
    switch (clickId) {
        ...
        case 'em-check-btn':
            ...
            let cmmContentType = 'application/json', // 콘텐츠 타입 
                cmmType = 'post', // 전송 방식
                cmmUrl = '/api/boapphome/emCheckTimeInput',
                cmmReqDataObj = { // 서버로 넘겨줄 데이터
                    emCode: emCode,
                    p_user_code: pUserCode,
...
function check(click) {
    ...
    switch (clickId) {
        case 'check-detail-btn':
            ...
            let cmmContentType = 'application/json', // 콘텐츠 타입 
                cmmType = 'get', // 전송 방식
                cmmUrl = '/api/boapphome/checkDetail',
                cmmReqDataObj = { // 서버로 넘겨줄 데이터
        ...
        case 'check-check-btn':
            ...
            let cmmContentType2 = 'application/json', // 콘텐츠 타입 
                cmmType2 = 'post', // 전송 방식
                cmmUrl2 = '/api/boapphome/emCheckTimeInput',
